/** Градация стеноза ВСА — табл. 4.1 Куликов / Grant E.G. et al., 2003 (parity с web). */

const ICA_PSV_NORMAL_MAX_CM_S = 125;
...
export function gradeCarotidStenosis(input: CarotidStenosisInput): CarotidStenosisResult {
  ...
  if (psv != null) {
    ...
    } else if (psv >= ICA_PSV_NORMAL_MAX_CM_S) {
      grade = "moderate";
...
export function buildCarotidStenosisReport(input: CarotidStenosisInput, result: CarotidStenosisResult): string {
  const lines = [
    "УЗД брахиоцефальных артерий — градация стеноза ВСА (допплер)",
    "",
    `PSV ВСА: ${input.psvIcaCmS ?? "—"} см/с`,
    `EDV ВСА: ${input.edvIcaCmS ?? "—"} см/с`,
    `PSV ОСА: ${input.psvCcaCmS ?? "—"} см/с`,
    input.morphologicPercent != null ? `Морфология (NASCET): ~${input.morphologicPercent}%` : null,
    input.occlusionSuspected ? "Подозрение на окклюзию: да" : null,
    "",
    `Заключение: ${result.label} (${result.percentRange})`,
    ...result.criteria.map((c) => `• ${c}`),
    result.strokeRiskNote,
    "",
    "Интерпретация — лечащий врач. Не заменяет клиническое решение.",
  ];
