...
/** Пучки гладкой мышцы — продольные волокна на поверхности миометрия */
function outerSurfaceRipple(theta: number, y: number): number {
  return (
    1 +
    0.038 * Math.sin(5 * theta + y * 3.2) +
    0.022 * Math.sin(12 * theta - y * 2.1) +
    0.048 * Math.sin(y * 9.4 + theta * 1.8) +
    0.014 * Math.cos(18 * theta + y * 0.9)
  );
}

/** Волнистая слизистая оболочка полости */
function innerEndometrialRipple(theta: number, y: number): number {
  return (
    1 +
    0.092 * Math.sin(18 * theta + y * 4.35) +
    0.058 * Math.sin(12 * theta - y * 3.4) +
    0.04 * Math.sin(26 * theta + 1.85 * y) +
    0.028 * Math.cos(8 * y + theta * 5)
  );
}
...
export function createSagittalCrossSectionCapGeometry(
  ...
  function innerWave(y: number): number {
    return 1 + 0.078 * Math.sin(17.2 * y + 0.38) + 0.048 * Math.sin(24.8 * y - 1.12) + 0.022 * Math.cos(9.5 * y);
  }
  ...
  function bandVertexColor(x: number, y: number, band: Band): THREE.Color {
    ...
    if (band === "endo") {
      ...
      c.setRGB(THREE.MathUtils.lerp(0.55, 0.92, t), THREE.MathUtils.lerp(0.18, 0.38, t), THREE.MathUtils.lerp(0.28, 0.42, t));
      return c;
    }
    const d = radialDepth01(Math.abs(x), y);
    if (d > 0.75) c.setHex(0xe8a8b0);
    else if (d > 0.45) c.setHex(0xd47888);
    else c.setHex(0xc45a6c);
    return c;
  }
...
/** Полное тело матки (360°) — объём для 3/4 ракурса */
export function createFullLatheOuterGeometry(segments = 80): THREE.BufferGeometry {
  const pts = OUTER_PROFILE.map((p) => new THREE.Vector2(p.x, p.y));
  const geo = new THREE.LatheGeometry(pts, segments);
  geo.computeVertexNormals();
  return geo;
}

/** Канал шейки + верхняя часть влагалища (сагиттальный разрез, z ≈ 0) */
export function createCervixCanalGeometry(rings = 28): THREE.BufferGeometry {
  const verts: number[] = [];
  const uvs: number[] = [];
  const indices: number[] = [];
  const colors: number[] = [];
  const col = new THREE.Color(0xd8a0a8);
  ...
  for (let j = 0; j <= rings; j++) {
    ...
    const ridge = 1 + 0.12 * Math.sin(22 * y + 0.5);
    const xInL = -0.06 * ridge;
    const xInR = 0.06 * ridge;
    const row = [
      [xOutL, y, "wall"],
      [xInL, y, "canal"],
      [xInR, y, "canal"],
    ...
    for (let i = 0; i < row.length; i++) {
      const [x, yy, kind] = row[i];
      verts.push(x, yy, 0.002);
      uvs.push(i / 3, t);
      if (kind === "canal") col.setHex(0xf0c4c8);
      else col.setHex(0xcc7a88);
      colors.push(col.r, col.g, col.b);
    }
  }
