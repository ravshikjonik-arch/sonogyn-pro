...
export async function POST(request: Request, { params }: Params) {
  const limited = await rejectIfRateLimitedPreset(request, "case-like", RL.syncBurst);
  ...
  const userRl = await rejectIfRateLimitedForUser(auth.userId, "case-like", RL.syncBurst);
  ...
  if (error) {
    safeLog("case like error", { code: error.code, message: error.message });
    return NextResponse.json({ error: "Не удалось сохранить лайк" }, { status: 500 });
  }
