...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      description,
      type: "website",
      url: `${appUrl}/tools/refs/courses/${courseId}`,
      images: course.coverUrl ? [{ url: course.coverUrl }] : undefined,
    },
  };
}

export default async function RefsCourseDetailPage({ params }: PageProps) {
  const { courseId } = await params;
  const supabase = await createClient();
  const course = await fetchPublishedCourseDetail(supabase, courseId);
  ...
  return (
    <div className="px-4 py-10 lg:px-10">
      <div className="mx-auto max-w-5xl space-y-4">
        <Link href="/tools/refs/courses" className="text-sm text-[var(--clinical-primary)] underline">
          ← Каталог курсов
        </Link>
        {jsonLd ? (
