...
/** PSV ВСА: верхняя граница «нормы» / порог умеренного стеноза (см/с). */
export const ICA_PSV_NORMAL_MAX_CM_S = 125;
...
export function buildCarotidStenosisReport(
  input: CarotidStenosisInput,
  result: CarotidStenosisResult,
): string {
  const lines = [
